...
if __name__ == '__main__':
	...
	frame_path = '/saat/anet_frames_nas_mount' #  /saat/Charades_for_SAAT/Charades_frames # make dir
	feat_path = '/saat/ActivityNet_roi_feats'# /saat/Charades_for_SAAT/roi_feat_fc # make dir
	# anno_mapping = '/saat/charades_train_mapping_for_vid_cap_for_3d_{}.json'.format(str(opt.cnt))
	anno_mapping = '/saat/anet_train_anno.json'
	...
	for vid in tqdm(vid_names):
		...
		for seg in segments:
			...
			fr_start = start_n * fr_len + 1
			fr_end = end_n * fr_len + 1
			...
			tmppath = os.path.join(curr_fr_path, str(half_fr).zfill(5) + '.png')
