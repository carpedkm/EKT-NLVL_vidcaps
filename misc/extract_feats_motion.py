...
def extract_feats(opt, file_path, net, filenames, frame_num, batch_size, save_path, dataset_name, anno):
	...
	for file in tqdm(filenames):
		...
		for cnt, bd_ in enumerate(bd_info):
			...
			end_time = bd_['end']
			feat_file = os.path.join(save_path, file[:-4] + '_' + str(cnt) + '.npy')
...
if __name__ == "__main__":
	...
	extract_feats(opt, dir_for_vid, model, namelist_to_pass, opt.frame_per_video, opt.batch_size, save_path, opt.dataset_name, anno)
