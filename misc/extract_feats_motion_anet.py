...
def extract_feats(opt, file_path, net, filenames, frame_num, batch_size, save_path, dataset_name, anno):
	...
	for file in tqdm(filenames):
		...
		for bd_ in bd_info:
			...
			# print('file name', file, 'bd_info', bd_info)
			...
			# print(idx)
			for i in idx:
				img = transform(np.array(Image.open(os.path.join(vid_dir, str(i).zfill(5) + '.png'))))
...
if __name__ == "__main__":
	...
	parser.add_argument('--file_path', type=str, default='/saat/ActivityNet_for_SAAT') 
	parser.add_argument('--dataset_name', type=str, default='ActivityNet') 
	...
	dir_for_vid = '/saat/anet_frames_nas_mount'
