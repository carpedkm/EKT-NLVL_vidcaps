...
if __name__ == '__main__':
	path_ = '/saat/anet_frames_nas_mount' #  /saat/Charades_for_SAAT/Charades_frames # make dir
	anno_path = '/saat/ActivityNet_for_SAAT/anet_train_anno.json'
	# not existing in RoI
	...
	charades_temp_fr_size = h5py.File('/saat/gtboundary_anet_h5py/anet_temp_fr_size.h5', 'w')

	cnt = 0
	for vid in tqdm(list(anno.keys())):
		# print('{}/({}, {})'.format(cnt, opt.start_idx, opt.end_idx))
		curr_fr_path = os.path.join(path_, vid + '.mp4')
		segcnt = len(anno[vid]['timestamps'])
	
		for i in range(segcnt):
			tmppath = os.path.join(curr_fr_path, str(1).zfill(5) +'.png')
			print(tmppath)
